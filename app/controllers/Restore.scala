...
class Restore(
  ...
  def userFromPandaUser(user: PandaUser) =
  ...
  def restore(
    ...
    if (
      !permissions.hasPermission(Permissions.RestoreContent, request.user.email)
    ) {
      Future.successful(
        Forbidden(
          s"You do not have the ${Permissions.RestoreContent.name} permission which is required to restore content"
        )
      )
    } else if (
      sourceId != destinationId && !permissions.hasPermission(
        Permissions.RestoreContentToAlternateStack,
        request.user.email
      )
    ) {
      Future.successful(
        Forbidden(
          s"You do not have the ${Permissions.RestoreContentToAlternateStack.name} permission which is required to restore content from one stack to another"
        )
      )
    } else {
      ...
      val result = snapshotApi
        .getSnapshot(sourceStack.snapshotBucket, snapshotId)
        .flatMap[Result] {
          case None           => Attempt.Right(NotFound)
          case Some(snapshot) =>
            flexibleApi
              .restore(targetStack, user, contentId, snapshot)
              .map(Ok(_))
        }
      result.fold(
  ...
  def restoreDestinations(contentId: String) = AuthAction.async {
    ...
    val destinations = config.allStacks.map { stack =>
      ...
      flexibleApi.changeDetails(stack, contentId).map { cdOption =>
        ...
        case NonFatal(e) =>
          logger.warn(
            s"Couldn't communicate with Flexible stack at ${stack.apiPrefix}",
            e
          )
          destination
      }
