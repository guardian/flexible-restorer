...
import play.routes.compiler.InjectedRoutesGenerator.Dependency
...
object PlambdaInjectedRoutesGenerator extends RoutesGenerator {
  ...
  private def generateReverseRouters(sourceInfo: RoutesSourceInfo, namespace: Option[String], additionalImports: Seq[String], routes: List[Route], namespaceReverseRouter: Boolean) = {
    routes.map{
      case Route(verb, path, HandlerCall("controllers", "Assets", instantiate, method, parameters), comments) =>
        Route(verb, path, HandlerCall("controllers", "Assets", instantiate, method, parameters), comments)
      case other => other
    }.groupBy(_.call.packageName).map {
      case (pn, routes) =>
